...
#[command(about = "OpenAI-compatible HTTP passthrough API")]
pub struct ProxyCommand {
    ...
    pub allow_cors_any: bool,

    /// Config overrides (merged into ~/.codex/config.toml)
    #[clap(flatten)]
    pub config: CliConfigOverrides,
}
...
pub async fn run(opts: ProxyCommand) -> anyhow::Result<()> {
    ...
    let upstream_display = upstream_base_v1.clone();
    ...
    let mut router = Router::new()
        ...
        .route("/v1", any(proxy_v1))
        .route("/v1/*tail", any(proxy_v1));

    // CORS + Trace
    let cors = if opts.allow_cors_any {
        CorsLayer::new()
            .allow_origin(Any)
            .allow_methods(Any)
            .allow_headers(Any)
    ...
    let listener: TcpListener = TcpListener::bind(&opts.bind)
        .await
        .with_context(|| format!("bind {}", opts.bind))?;
    let addr: SocketAddr = listener.local_addr()?;
    info!("codex-proxy listening on http://{}", addr);
    println!(
        "codex proxy listening on http://{addr} (health: /health, proxying: /v1/* -> {upstream_display})"
    );
